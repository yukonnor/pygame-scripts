...
night_sky = pygame.image.load("cowboy_milkyway/night_sky.png")
...
hat = pygame.image.load("cowboy_milkyway/cowboy-hat-2.png")
...
face = pygame.image.load("cowboy_milkyway/mannequin-head.png")
...
shirt = pygame.image.load("cowboy_milkyway/shirt.png")
...
arm_left = pygame.image.load("cowboy_milkyway/arm-left-copy.png")
arm_left_pivot = [0, 0]
arm_right_pivot = [0, 0]
...
cloud = pygame.image.load("cowboy_milkyway/night_sky_cloud.png")
...
def rotate(surface, angle, pivot, offset):
    ...
    rotated_image = pygame.transform.rotozoom(
        surface, -angle, 1
    )  # Rotate the image, final arg is "scale"
...
def update_image_positions(hat_pivot, hat_scale):
    ...
    face_pivot[1] = hat_pivot[1] + (face_rect_orig.height * face_scale * 0.5)

    shirt_scale = hat_scale * 0.8
    shirt_pivot[0] = hat_pivot[0]
    shirt_pivot[1] = face_pivot[1] + (shirt_rect_orig.height * shirt_scale * 0.6)

    arm_left_pivot[0] = shirt_pivot[0] - (
        shirt_rect_orig.height * shirt_scale * 0.25
    )  # around left shoulder
    arm_left_pivot[1] = shirt_pivot[1] - (shirt_rect_orig.height * shirt_scale * 0.2)
    arm_right_pivot[0] = shirt_pivot[0] + (
        shirt_rect_orig.height * shirt_scale * 0.25
    )  # around right shoulder
    arm_right_pivot[1] = shirt_pivot[1] - (shirt_rect_orig.height * shirt_scale * 0.2)
...
def animate_hat_size():
    ...
    if hat_size[0] <= 100:
        hat_size_dir[0] = 1
    if hat_size[1] >= hat_size[0] * 0.5:
        hat_size_dir[1] = -1
    if (hat_size[1]) <= hat_size[0] * 0.3:
        hat_size_dir[1] = 1
...
def animate_hat_position():
    ...
    hat_pivot[0] += (
        hat_dir[0] * hat_speed[0]
    )  # center the hat horizonally (pivot point is in center of hat)
...
while True:
    ...
    screen.blit(
        scaled_night_sky, (0, 0)
    )  # clear screen with night sky background (transparent)
    ...
    cloud_center_y = int(shirt_pivot[1] + (shirt_rect_orig.height * shirt_scale) * 0.6)
    scaled_cloud_rect.center = (cloud_center_x, cloud_center_y)

    ##########################
    # Rotate Images
    rotated_arm_left, rotated_arm_left_rect = rotate(
        scaled_arm_left, arm_left_angle, arm_left_pivot, arm_left_pivot_offset
    )
    rotated_arm_right, rotated_arm_right_rect = rotate(
        scaled_arm_right, arm_right_angle, arm_right_pivot, arm_right_pivot_offset
    )
    rotated_shirt, rotated_shirt_rect = rotate(
        scaled_shirt, shirt_angle, shirt_pivot, shirt_pivot_offset
    )
    rotated_face, rotated_face_rect = rotate(
        scaled_face, face_angle, face_pivot, face_pivot_offset
    )
    rotated_hat, rotated_hat_rect = rotate(
        scaled_hat, hat_angle, hat_pivot, hat_pivot_offset
    )
    ...
    if False:
        ...
        circle_center = [
            shirt_pivot[0],
            shirt_pivot[1] + shirt_rect_orig.height * shirt_scale * 0.8,
        ]
