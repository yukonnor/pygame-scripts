...
def main():
    ...
    glTranslatef(0.0, 0.0, -5)     # x,y,z params for object's position (or camera's?)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    glTranslatef(-0.2, 0.0, 0.0)     # x,y,z params for object 
            ...
            if event.type == pygame.MOUSEWHEEL:
                if event.x == 1:  # rotate left
                    glRotatef(2, 0, 1, 0) 
                if event.x == -1:  # rotate right
                    glRotatef(-2, 0, 1, 0) 
     
                
        #glRotatef(1, 3, 2, 1) # set object to rotate once every frame      
