...
FILEPATH = "moire/images/"
SVG_NAMES = [
    "cirlces.svg",
    "lines_0.svg",
    "lines_1.svg",
    "radial_angles_0.svg",
    "halftone_linear_3.svg",
    "halftone_linear_4.svg",
    "halftone_donut.svg",
    "halftone_rings.svg",
]
SVG_WIDTH = 1000
...
btm_img_rect = (
    btm_img_svg.get_rect()
)  # this doesn't appear to ingest the scaleed size, so have to adjust
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.unicode in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]:
                top_img_num = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].index(
                    event.unicode
                )
                top_img_svg, top_transformed_img = change_image(
                    top_img_num, top_scale, top_color, top_angle
                )
            ...
            if event.unicode in ["!", "@", "#", "$", "%", "^", "&", "*", "(", ")"]:
                ...
                btm_img_svg, btm_transformed_img = change_image(
                    btm_img_num, btm_scale, btm_color, 0
                )
    ...
    if change_top_scale:
        ...
        top_transformed_img = pygame.transform.rotozoom(
            top_img_svg, top_angle, top_scale
        )
        top_img_rect = top_transformed_img.get_rect()
        top_img_rect.center = (top_x, top_y)  # latest x,y position
        change_top_scale = False
